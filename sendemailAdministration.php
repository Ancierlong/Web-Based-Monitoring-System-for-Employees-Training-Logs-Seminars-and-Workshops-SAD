...
$sql = "SELECT ID, title, dateofseminar, venue, remarks, classify, empid FROM employeelogs WHERE classify LIKE 'Administration' ORDER BY dateofseminar DESC";
...
while ($row2 = $result2->fetch_assoc()) {
    ...
    $table .= '<td style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">' . $row2['classify'] . '</td>';
    $table .= '</tr>';
    }
}
...
try {
    ...
    $mail->setFrom('');
    $mail->addAddress('');
    $mail->Subject = 'Request for Training Logs - Administration Classification';
